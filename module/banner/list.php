...
if (mysqli_num_rows($queryBanner) == 0) {
    echo "<h3 class='text-red-600 font-semibold uppercase text-center text-2xl'>There Are Currently No Banners In The Database</h3>";
} else { ?>
            ...
            <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                <div class="overflow-hidden">
                    ...
                    if ($notif == "true") { ?>
                        ...
                        <?php echo "<meta http-equiv='refresh', content='2; url=index.php?page=my_profile&module=banner&action=list'>";
                    } ?>
                        </div>
                        ...
                        ?>
                            <tbody>
                                <tr class="border-b border-neutral-500">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4"><?php echo $row['banner'] ?></td>
                                    <td class="whitespace-nowrap px-6 py-4"><?php echo $row['link'] ?></td>
                                    <td class="whitespace-nowrap px-9 py-4"><?php echo $row['status'] ?></td>
                                    <td class="whitespace-nowrap py-4">
                                        <div class="flex space-x-2">
                                            <a title="Edit" href="index.php?page=my_profile&module=banner&action=form&banner_id=<?php echo $row['banner_id'] ?>" class="inline-block px-3 py-2.5 bg-blue-500 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-600 hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-700 active:shadow-lg transition duration-150 ease-in-out">
                                            ...
                                            <a title="Delete" href="module/banner/proses_hapus.php?banner_id=<?php echo $row['banner_id'] ?>" class="inline-block px-3 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out">
                                                <svg fill="none" stroke="currentColor" class="w-5 h-5" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
<script>
    $(document).ready(function() {
        $("#search").on("keyup", function() {
            ...
            $("#container").load("module/banner/livedata/data.php?search=" + search);
        });
    });
</script>